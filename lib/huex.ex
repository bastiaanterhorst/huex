defmodule Huex do
  ...
  * Command functions return a `Bridge` struct in order to be pipeline friendly.
  ...
  def nudge_group_brightness(bridge, group, brightness_inc) do
    bridge |> set_group_state(group, %{on: true, bri_inc: round(brightness_inc * 255.0)})
  end
  
  @doc """
  In- or decrement the brigthness of the given group (a value between -1 and 1) using the given transition time (in ms).
  Requires the connection to be authorized.
  """
